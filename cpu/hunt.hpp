...
struct VecHunt {
  ...
  operator()(double x) {
    if ((x - xx.front()) * (x - xx.back()) > 0) {
      printf("VecHunt::operator(): x = %g is out of bounds [%g, %g]: %d\n", x, xx.front(), xx.back(), (int)xx.size());
    }
    int n = xx.size();
...
struct UniformHunt {
  ...
  UniformHunt(std::string const& fname) {
  ...
  operator()(double& x) {
    ...
    if (x <= x_min) {
      if (x < x_min) printf("UniformHunt::operator(): x = %g is out of bounds [%g, %g]: %d\n", x, x_min, x_max, n);
      i = 0;
      a = 0;
      x = x_min;
      return std::make_pair(i, a);
    ...
    if (x >= x_max) {
      if (x > x_max) printf("UniformHunt::operator(): x = %g is out of bounds [%g, %g]: %d\n", x, x_min, x_max, n);
      i = n - 2;
      a = 1;
      x = x_max;
      return std::make_pair(i, a);
